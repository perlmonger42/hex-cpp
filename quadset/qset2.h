...
// This is qset<BITS,2>, a bitset template class specialized for holding two
// quadwords' worth of bits -- i.e., between 65 and 128 bits.
...
public:
  ...
  static_assert (65 <= BITS && BITS <= 128,
      "qset<BITS,2>: BITS must be in 65..128");
  ...
  inline constexpr bitpos min() const {
    if (b0 != 0) {
      ...
      return lowestBitPosition(b1) + 64;
    } else {
      panic("qset2.min(): cannot find any element in the empty set");
      return 128;
    }
  }
  ...
  inline constexpr bitpos max() const {
    if (b1 != 0) {
      return highestBitPosition(b1) + 64;
    } else if (b0 != 0) {
      return highestBitPosition(b0);
    } else {
      panic("qset2.max(): cannot find any element in the empty set");
      return 128;
    }
  }
  ...
  inline constexpr bitpos count() const {
    return countBits(b0) + countBits(b1);
  }
  ...
  inline constexpr qSet operator<<(bitpos n) const {
    if (n < 0) {
      ...
      return qSet{b0 << n, (b1 << n | b0 >> (64-n)) & bit_mask(0, BITS-65)};
    } else if (n == 64) {
      return qSet{0, b0 & bit_mask(0, BITS-65)};
    } else if (n < 128) {
      return qSet{0, (b0 << (n - 64)) & bit_mask(0, BITS-65)};
    } else {
      return qSet{0};
    }
  }
  ...
  inline constexpr qSet operator>>(bitpos n) const {
    if (n < 0) {
      ...
      return qSet{b0 >> n | b1 << (64-n), b1 >> n};
    } else if (n == 64) {
      return qSet{b1, 0};
    } else if (n < 128) {
      return qSet{b1 >> (n - 64), 0};
    } else {
      return qSet{0};
    }
  }
  ...
  inline constexpr qSet operator& (qSet other) const {
    return qSet{b0 & other.b0, b1 & other.b1};
  }
  ...
  inline constexpr qSet& operator<<=(bitpos n) {
    if (n < 0) {
      ...
      b1 = (b1 << n | b0 >> (64-n)) & bit_mask(0, BITS-65);
      b0 = b0 << n;
    } else if (n == 64) {
      b1 = b0 & bit_mask(0, BITS-65);
      b0 = 0;
    } else if (n < 128) {
...
static_assert (std::is_pod< qset<128,2> >::value, "qset<BITS,2> must be POD");
